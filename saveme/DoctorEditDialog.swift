...
open class DoctorEditDialog: UIView {
    ...
    @objc func deviceOrientationDidChange(_ notification: Notification) {
        ...
        let dialogSize = CGSize(width: 300, height: 280 + kDefaultButtonHeight + kDefaultButtonSpacerHeight)
        dialogView.frame = CGRect(x: (UIScreen.main.bounds.size.width - dialogSize.width) / 2,
                                  y: (((UIScreen.main.bounds.size.height - dialogSize.height) / 2)-100),
    ...
    fileprivate func configureLayer() -> DoctorEdit {
        let contentFrame = CGRect(x:0, y:40+kDefaultButtonHeight, width: 300, height: 234)
        let result : DoctorEdit! = DoctorEdit(frame: contentFrame)
        return result
    }
    ...
    @objc func buttonTapped(sender: UIButton!) {
        ...
        let email: String = self.slayer.email.text!
        let phone: String = self.slayer.phone.text!
        let custom: String = self.slayer.custom.text!
        let special: String = self.slayer.special.text!
        
        if self.item == nil {
            // new one
            let it = DoctorModel(name, email, phone, custom, special)
            DataStore.shared.userData?.doctors.append(it)
        } else {
            // edited
