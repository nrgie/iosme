...
struct Country {
    ...
    var flag: UIImage? {
        ...
        let bundlePath = Bundle.main.path(forResource: "SwiftCountryPicker", ofType: "bundle")
        let bundle = Bundle(path: bundlePath!)
        
        return UIImage(named: "Images/\(code.uppercased())", in: bundle, compatibleWith: nil)
    }
...
open class MRCountryPicker: UIPickerView, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    open func getSelected() {
        let country = countries[self.selectedRow(inComponent: 0)]
        if let countryPickerDelegate = countryPickerDelegate {
                    print("getSelect triggered")
